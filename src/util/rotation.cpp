...
Rotation::Rotation(Radians angle, const RealRect &rect, double zoom, double stretch, RotationFlags flags)
    : angle(constrain_radians(angle)), size(rect.size()), origin(rect.position()), zoomX(zoom * stretch), zoomY(zoom)
{
  if (stretch != 1.0)
  {
    size.width /= stretch;
...
wxRegion Rotation::trRectToRegion(const RealRect &r) const
{
  ...
  {
    wxPoint points[4] = {trPixel(RealPoint(r.left(), r.top())), trPixel(RealPoint(r.left(), r.bottom())), trPixel(RealPoint(r.right(), r.bottom())), trPixel(RealPoint(r.right(), r.top()))};
...
RealPoint Rotation::boundingBoxCorner(const RealSize &size) const
{
  ...
  if (angle <= rad90)
    return RealPoint(0, -w * s);
  if (angle <= rad180)
    return RealPoint(w * c, h * c - w * s);
  if (angle <= rad270)
    return RealPoint(w * c + h * s, h * c);
  else
    return RealPoint(h * s, 0);
}
...
RotatedDC::RotatedDC(DC &dc, Radians angle, const RealRect &rect, double zoom, RenderQuality quality, RotationFlags flags)
    : Rotation(angle, rect, zoom, 1.0, flags), dc(dc), quality(quality)
{
}

RotatedDC::RotatedDC(DC &dc, const Rotation &rotation, RenderQuality quality)
    : Rotation(rotation), dc(dc), quality(quality)
{
}
...
{
  wxRect r_ext = trRectToBB(r);
  if (r_ext.width <= 0 || r_ext.height <= 0)
    return;
  wxBitmap bmp(r_ext.width, r_ext.height, 24);
  if (!bmp.IsOk())
    return; // Bitmap creation failed
  ...
  if (!data)
    return; // Raw access not available on this platform/bitmap
