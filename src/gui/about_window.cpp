...
AboutWindow::AboutWindow(Window *parent)
    : wxDialog(parent, wxID_ANY, _TITLE_("about"), wxDefaultPosition, wxSize(510, 460), wxCLIP_CHILDREN | wxDEFAULT_DIALOG_STYLE | wxTAB_TRAVERSAL), logo(load_resource_image(settings.darkModePrefix() + _("about")))
{
  // init controls
...
const char *MSE_AUTHORS[] = {
    ...
    "CaiCai (247321453)",
    "Christian Daniel Jensen (cdjensen94)"};
...
HoverButtonBase::HoverButtonBase(Window *parent, int id, bool accepts_focus)
    : wxControl(parent, id, wxDefaultPosition, wxDefaultSize, wxNO_BORDER), accepts_focus(accepts_focus), hover(false), focus(false), mouse_down(false), key_down(false)
{
...
void HoverButtonBase::onMouseEnter(wxMouseEvent &)
{
  ...
  refreshIfNeeded();
  if (!help_text.empty())
    set_status_text(this, help_text);
}
...
HoverButton::HoverButton(Window *parent, int id, const String &name, const Color &background, bool accepts_focus)
    : HoverButtonBase(parent, id, accepts_focus), background(background), last_drawn(nullptr)
{
  loadBitmaps(name);
